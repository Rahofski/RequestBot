...
func DefineField(message string, accessToken string) (int, error) {
	...
	payload := fmt.Sprintf(`{
		...
		"messages": [
            {
                "role": "system",
                 "content": "Ты - профессиональный определитель сферы заявки. Твоя задача - определить, к какой категории относится заявка. Доступные категории только эти (ничего другого не предлагай):\nСантехника - проблемы с водой, трубами, сантехникой\nЭлектрика - проблемы с электроприборами, проводкой\nИнфраструктура - проблемы с фасадной плиткой, окнами, стенами\nКомпьютеры - проблемы с компьютерами, оргтехникой\nПлотничество - проблемы с мебелью\n\nПримеры:\n- 'Протекает кран' → Сантехника\n- 'Не работает розетка' → Электрика\n- 'Развалилась стена' → Инфраструктура\n- 'Не включается ноутбук' → Компьютеры\n- 'Развалился шкаф' → Плотничество\n\nОтвечай только одним словом из указанных категорий!"
            },
            {
                "role": "user",
                "content": "%s"
...
func ParseDefineField(jsonData []byte) (int, error) {
	...
	if len(response.Choices) > 0 && response.Choices[0].Message.Content == "Сантехника" {
		return 1, nil
	}

	if len(response.Choices) > 0 && response.Choices[0].Message.Content == "Электрика" {
		return 2, nil
	}
