...
func DefineField(message string ,accessToken string) (int, error) { 
	...
	payload := fmt.Sprintf(`{
		...
		"messages": [
		  {
			"role": "system",
			"content": "Ты - професстональный определитель сферы заявки. Твоя задача - определить, к какой категории относится заявка. Доступные категории: Водоснабжение , Электроснабжение , Газоснабжение , Компютеры, Плотничество. В качестве ответа верни одно слово "
		  },
		  {
			"role": "user",
			"content": "%s"
...
func ParseDefineField(jsonData []byte) (int, error) {
	...
	if len(response.Choices) > 0 && response.Choices[0].Message.Content == "Электроснабжение" {
		return 2, nil
	}

	if len(response.Choices) > 0 && response.Choices[0].Message.Content == "Газоснабжение" {
		return 3, nil
	}
